...
exports.disrepencyFromData =`create TABLE disrepency  As (select (facedrive.ride_id) as Discrepency_ID, (stripe.id) as Stripe_Charge_ID, ('new') as Status , ('no disrepency') as Description, (NULL) as Notes, stripe.amount, facedrive.up_amount_charged, (facedrive.up_amount_charged - stripe.amount ) as Desrepency_Amount,  (stripe.created_utc) as Date from facedrive,stripe where ((facedrive.up_amount_charged - stripe.amount ) > 0) and facedrive.stripe_reserve_charge_id = stripe.source );`

exports.addPrimaryKey=`ALTER TABLE disrepency ADD CONSTRAINT ds_pk PRIMARY KEY (Discrepency_ID);`